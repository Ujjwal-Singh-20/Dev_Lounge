...
import { Send, Smile, Hash, Command, Settings } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const ChatRoom = () => {
    const { currentRoom, messages, sendMessage, currentUser } = useChat();
    ...
    const isCreator = currentRoom?.creatorEmail === currentUser?.email;
                ...
                <div className="flex items-center gap-4">
                    {isCreator && (
                        <motion.button
                            ...
                            className="p-2 text-gray-400 hover:text-brand-primary hover:bg-brand-primary/10 rounded-xl transition-all"
                            title="Room Settings"
                        >
                            <Settings size={18} />
                        </motion.button>
                    ...
                    <div className="flex items-center gap-1.5 bg-green-500/10 px-2 py-1 rounded-lg border border-green-500/20">
                        ...
                        <span className="text-[10px] uppercase tracking-wider font-bold text-green-600/80">Live</span>
                    </div>
                </div>
            </motion.div>
            ...
            >
                ...
                >
                    <div className="relative flex items-end gap-2 glass-premium p-2 rounded-2xl focus-within:ring-4 ring-brand-primary/10 transition-all duration-500 group-hover:bg-white/10 dark:group-hover:bg-white/[0.07]">
                        {/* Empty space where Paperclip was */}
                        <div className="w-2" />
