...
const CollaborativeEditor = ({ theme }) => {
    ...
    const handleCreateFile = async (filename, language, content = '') => {
        ...
        if (filesToUpload.length > 0) {
            ...
            reader.onload = async (event) => {
                ...
                try {
                    ...
                    console.error('Failed to create file during drop:', err);
                }
            };
    ...
    return (
        <RoomProvider id={currentRoom.id} initialPresence={{ cursor: null }}>
            <div
                ...
                className="flex-1 flex flex-col h-full bg-gray-50 dark:bg-chat-bg relative overflow-hidden"
            >
                {/* ── Editor Header ───────────────────────────────────────────── */}
                <motion.div
