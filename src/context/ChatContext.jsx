...
import { setLiveblocksAuthToken } from '../liveblocks.config';
...
export const ChatProvider = ({ children }) => {
    ...
    useEffect(() => {
        setLiveblocksAuthToken(currentUser?.idToken);
    }, [currentUser]);
    ...
    }, [authHeaders, currentRoom]);

    const updateRoomAccess = useCallback(async (roomId, allowedEmails) => {
        const res = await fetch(`${API_BASE}/rooms/${roomId}/access`, {
            method: 'PATCH',
            headers: authHeaders(),
            body: JSON.stringify({ allowedEmails }),
        });
        const room = await res.json();
        if (!res.ok) throw new Error(room.error || 'Failed to update access');
    ...
    const value = {
        ...
        // Rooms
        rooms, currentRoom, setCurrentRoom: selectRoom, createOrJoinRoom, joinRoom, fetchRooms, deleteRoom, updateRoomAccess,
        // Chat
